...
const CHECKLIST_E1 = [
  ...
  // INSTALACIONES
  { id:"e1_14", cat:"Inst. Hidráulica",  code:"IH.1", label:"Instalación hidráulica colocada al 100%" },
  { id:"e1_15", cat:"Inst. Hidráulica",  code:"IH.2", label:"Pasos en contratrabes, trabes y losas hidráulicos conforme a proyecto" },
  { id:"e1_17", cat:"Inst. Sanitaria",   code:"IS.1", label:"Instalación sanitaria oculta concluida" },
  { id:"e1_18", cat:"Inst. Sanitaria",   code:"IS.2", label:"Pasos sanitarios en contratrabes, trabes y losas conforme a proyecto" },
  { id:"e1_19", cat:"Inst. Sanitaria",   code:"IS.3", label:"Registros y tuberías protegidos para evitar derrame de concreto" },
  { id:"e1_20", cat:"Inst. Pluvial",     code:"IP.1", label:"Instalación pluvial oculta concluida" },
  { id:"e1_21", cat:"Inst. Pluvial",     code:"IP.2", label:"Pasos pluviales en contratrabes, trabes y losas conforme a proyecto" },
  { id:"e1_22", cat:"Inst. Pluvial",     code:"IP.3", label:"Registros y tuberías pluviales protegidos; tubos sin deformaciones" },
  ...
  { id:"e1_28", cat:"Geometría",   code:"G.3",  label:"Superficie de trabajo uniforme" },
];
...
const CHECKLIST_E2 = [
  ...
  { id:"e2_07", cat:"Cimbra",    code:"C.7",  label:"Pasos para instalaciones dejados según proyecto arquitectónico y de especialidades" },
  ...
  { id:"e2_12", cat:"Limpieza",  code:"L.4",  label:"Juntas de construcción con rugosidad y limpieza adecuadas" },
  ...
  const EPanel = ({ etapa, items, num, accent, accentBg, title }) => {
    ...
    return (
      <div>
        ...
        <div style={{ display: "flex", gap: 12, fontSize: 11, color: "#a8a29e", marginTop: 10, paddingTop: 10, borderTop: "1px solid #f5f4f3" }}>
          <span>📷 {etapa.fotos} fotos</span>
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 12, width: "100%", maxWidth: 760, maxHeight: "88vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,.15)" }}>
        ...
        <div style={{ padding: "20px 24px", borderBottom: "1px solid #f5f4f3", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "#78716c", marginTop: 1 }}>Residente: {record.residente}</div>
          </div>
...
function EtapaForm({ record, etapaNum, onSave, onCancel }) {
  ...
  const p = pct(cl);
  const canSave = p > 80 && firmaRes && firmaCont;
      ...
      {step === 2 && (
        <div>
          ...
          <label style={{ display: "block", border: `2px dashed ${accentBg === "#eff6ff" ? "#bfdbfe" : "#fde68a"}`, borderRadius: 10, padding: "32px 20px", textAlign: "center", cursor: "pointer", background: accentBg, marginBottom: 14, transition: "border .2s" }}>
            ...
            <input type="file" multiple accept="image/*" onChange={e => setFotos(prev => [...prev, ...Array.from(e.target.files).map(f => f.name)])} style={{ display: "none" }} />
          </label>
          {fotos.length > 0 && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 8, marginBottom: 12 }}>
              {fotos.map((f, i) => (
                <div key={i} style={{ background: "#fafaf9", border: "1px solid #e7e5e4", borderRadius: 6, padding: "7px 10px", fontSize: 11, color: "#78716c", display: "flex", gap: 6, alignItems: "center" }}>
                  <span>🖼️</span><span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {step === 3 && (
        <div>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "space-between" }}>
            ...
            <button onClick={() => onSave(record, etapaNum, { estado: p === 100 ? "aprobado" : "pendiente", checklist: cl, fotos: fotos.length, firmaResidente: firmaRes, firmaContratista: firmaCont, obs, fechaHora: new Date().toLocaleString("es-MX", { day:"2-digit", month:"2-digit", year:"numeric", hour:"2-digit", minute:"2-digit" }) })}
              disabled={!canSave}
...
function NewElementForm({ onSave, onCancel }) {
  const [d, setD] = useState({ tipoElemento: [], elemento: "", eje: "", nivel: "", fecha: new Date().toISOString().split("T")[0], residente: "" });
  ...
  const valid = d.tipoElemento.length > 0 && d.elemento && d.eje && d.nivel && d.residente;
  return (
...
// TARJETA DE ELEMENTO
// ─────────────────────────────────────────────────────────────────────────────
function Card({ record, onClick, onEtapa }) {
    ...
    <div onClick={() => onClick(record)}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "#a8a29e", marginTop: 2 }}>Eje {record.eje} · {record.nivel} · {record.fecha} · {record.residente}</div>
        </div>
        <StatusPill estado={globalEstado(record)} />
      </div>
  ...
  useEffect(()=>{
    (async()=>{
      try {
        ...
        setPins(snap.exists() ? snap.data() : { dueno:"0000", residente:"1111" });
      } catch(e){ setPins({ dueno:"0000", residente:"1111" }); }
    })();
  },[]);
...
export default function App() {
  ...
  const handleNew=async(datos)=>{
    ...
    const rec={...datos,createdAt:Date.now(),etapa1:{estado:"pendiente",checklist:emptyCheck(CHECKLIST_E1),fotos:0,firmaResidente:false,firmaContratista:false,obs:""},etapa2:null};
  ...
  const handleEtapaSave=async(record,num,data)=>{
    ...
    if(num===1){
      await updateDoc(ref,{etapa1:data,etapa2:{estado:"pendiente",checklist:emptyCheck(CHECKLIST_E2),fotos:0,firmaResidente:false,firmaContratista:false,obs:""}});
    } else {
  ...
  const counts={total:records.length,proceso:records.filter(r=>globalEstado(r)==="proceso").length,listo:records.filter(r=>globalEstado(r)==="listo").length};
        ...
        {view === "dash" && (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {filtered.map(r => (
                <Card key={r.id} record={r} onClick={setSelected} onEtapa={role === "residente" ? openEtapa : null} />
              ))}
            </div>
          </>
        )}
